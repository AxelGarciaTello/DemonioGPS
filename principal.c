...
// Se realiza un segundo fork para separarnos completamente de la sesion del padre
    ...
    if( pid )
    {
		...
		apArch = fopen("/home/axel/demonio.pid", "w");
...
void *funHilo(){
  ...
  while(enchufe){
    ...
    if(dato == '$'){
      read(fd_serie, &GPGGA[0], 1);
      read(fd_serie, &GPGGA[1], 1);
      read(fd_serie, &GPGGA[2], 1);
      read(fd_serie, &GPGGA[3], 1);
      read(fd_serie, &GPGGA[4], 1);
      if(GPGGA[0]=='G' &&
         GPGGA[1]=='P' &&
         GPGGA[2]=='G' &&
         GPGGA[3]=='G' &&
